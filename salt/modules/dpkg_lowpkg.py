...
def _get_pkg_install_time(pkg, arch):
    ...
    if pkg is not None:
        ...
        if arch is not None and arch != "all":
            locations.append(os.path.join(loc_root, f"{pkg}:{arch}.list"))

        locations.append(os.path.join(loc_root, f"{pkg}.list"))
        for location in locations:
            try:
                iso_time_t = int(os.path.getmtime(location))
                iso_time = (
                    salt.utils.timeutil.utcfromtimestamp(iso_time_t).isoformat() + "Z"
                )
                break
            except OSError:
                pass
